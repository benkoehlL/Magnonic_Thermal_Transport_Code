...
int main(){
    ...
    for(int count=1;count<=Dmax*100/*+1*/;count++){
        ...
        ostringstream of1;
        of1 << "Drudeweight/xsize" << xsize << "ysize" << ysize << "fieldsize" << fieldsize << "/drudeweightnumD" << D*100 << "cJxsize.dat";
        ...
        for(double T=0.001;T<0.5;T+=0.005){
            drudesumxx = {0.0,0.0};
            drudesumxy = {0.0,0.0};
            drudesumyx = {0.0,0.0};
            drudesumyy = {0.0,0.0};
            ...
            for(int alpha=0;alpha<2*systemsize;alpha++){
                for(int beta=0;beta<2*systemsize;beta++){
                    ...
                    drudesumxx += bosealpha*bosebeta*(prod1xx+prod2xx)/(omega - energydiff + T*T*T*T*cutoffepsilon);
                    drudesumxy += bosealpha*bosebeta*(prod1xy+prod2xy)/(omega - energydiff + T*T*T*T*cutoffepsilon);
                    drudesumyx += bosealpha*bosebeta*(prod1yx+prod2yx)/(omega - energydiff + T*T*T*T*cutoffepsilon);
                    drudesumyy += bosealpha*bosebeta*(prod1yy+prod2yy)/(omega - energydiff + T*T*T*T*cutoffepsilon);
                }
            }
