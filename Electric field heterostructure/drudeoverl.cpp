...
int main(){
    ...
    for(int t=1;t<4;t++){
        double T = t*0.1;
        for(int count=10;count<=Dmax*100/*+1*/;count+=10){
            D = count*0.01;
            ostringstream of;
            of << "Drudeoverl/drudeweightD" << count << "cJxsize" << xsize << "ysize" << ysize << "T" << T*100 << "cJ.dat";
            ofstream writel(of.str().c_str());//add when using binary format: ,ios_base::binary
            for(int l=0;l<52;l+=2){
                ...
                ostringstream fin1;
                fin1 << "Texture/textureD" << D*100 << "cJxsize" << xsize << "ysize" << ysize << "fieldsize" << l << ".dat";
                ...
                ostringstream fin2;
                fin2 << "Bogoliubov/eigenvectorD" << D*100 << "cJxsize" << xsize << "ysize" << ysize << "fieldsize" << l << ".dat";
                ...
                writel << l << '\t' << imag(drudesumxx/((complex<double>)(systemsize))) << '\t' << imag(drudesumxy/((complex<double>)(systemsize)))<< '\t' << imag(drudesumyx/((complex<double>)(2*systemsize)))<< '\t' << imag(drudesumyy/((complex<double>)(systemsize)))<< '\n';
