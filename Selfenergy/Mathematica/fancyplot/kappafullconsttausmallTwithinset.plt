...
set xrange [0:0.3]
set yrange [0:0.1]
set xtics 0,0.02,0.3  scale 1.5 ,0.75 format ''
set mxtics 2
set xtics add ('\psa{0}' 0 , '\psa{0.1}' 0.1, '\psa{0.2}' 0.2)
set ytics 0,0.0125,0.1 scale 1.5 ,0.75 format '' 
set mytics 2
#set ytics add ('\psa{0.025}' 0.025, '\psa{0.05}' 0.05, '\psa{0.075}' 0.075)
set xlabel ''
set ylabel '' offset 0,0
unset key

plot 'BfieldtauconstTdep.dat' u 1:(0.5*$2) w l lw 7.5 t sprintf("$B$ = %0.1f $B_S$",0.0), 'BfieldtauconstTdep.dat' u 1:($5) w l lw 7.5 t sprintf("$B$ = %0.1f $B_S$",0.3), 'BfieldtauconstTdep.dat' u 1:($7) w l lw 7.5 t sprintf("$B$ = %0.1f $B_S$",0.5), 'BfieldtauconstTdep.dat' u 1:($10) w l lw 7.5 t sprintf("$B$ = %0.1f $B_S$",0.8)
