...
int main(){
    ...
    //while(b<1.0){
        ...
        fout << "SelfenergyBconst/" << b*100 << "BsvariousTemperatures/tempselfenergyT" << T*100 << "cJ.dat";
        ...
        for(int kx=0; kx<ksize; kx++){
            for(int ky=0; ky<ksize; ky++){
                ...
                for(int qx=0; qx<2*qsize; qx++){
					 ...
					 -M_PI,(ky*(M_PI/kpi)+qy*(M_PI/qpi))-M_PI,b);
                            if(((abs(kx*qpi-qx*kpi)!=2*kpi*qpi) && (abs(ky*qpi-qy*kpi)!=2*qpi*kpi) && (abs(kx*qpi-qx*kpi)!=0) && (abs(ky*qpi-qy*kpi)!=0))  && ((qx != qpi) 
				|| (qy != qpi)) && ((kx != kpi) || (ky != kpi))){
                                energy = dispersion(kx*M_PI/kpi,ky*M_PI/kpi,b) + dispersion(qx*M_PI/qpi,qy*M_PI/qpi,b) - dispersion((kx*(M_PI/kpi)-qx*(M_PI/qpi))
					 +M_PI,(ky*(M_PI/kpi)-qy*(M_PI/qpi))+M_PI,b);
                                nonvertex = (bosefunction(qx*M_PI/qpi,qy*M_PI/qpi,b,1.0/T)-bosefunction((kx*(M_PI/kpi)-qx*(M_PI/qpi))+M_PI,(ky*(M_PI/kpi)-qy*(M_PI/qpi))
						+M_PI,b,1.0/T))/(energy + epsilon);
