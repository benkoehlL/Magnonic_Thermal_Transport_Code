...
int main(){
    ...
    while(b<1.0){
        ...
        read.close();
        ostringstream fout;
        fout << "Reducedthermalconductivity/reducedthermalconductivityB" << b*1000 << "mBs.dat";
                    ...
                    for(int ky=0; ky<kspacesize; ky++){
                        if((kx!=kspacesize-1) || (ky!=kspacesize-1)){
                                ...
                                if(help[kx*kspacesize+ky]>10*gbscatteringrate){
                                    reducedsum = reducedsum + heatconductivitydensity(kx*M_PI/(kspacesize-1),ky*M_PI/(kspacesize-1),b,1.0/(Tstep*T),J*gbscatteringrate)/(kspacesize*kspacesize);
                                }
                        }
                    }
                }
                write << T*Tstep << '\t' << reducedsum/sum << '\n';
            }
        }
        else{
            for(int T=1; T<1.0/Tstep;T++){
