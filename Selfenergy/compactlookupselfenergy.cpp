...
int main(){
    ...
    while(b<1.0){
        ostringstream fout;
        fout << "Compactselfenergy/compactselfenergyB" << b*1000 << "mBsT" << Tbfield*100 << "cJ.dat";
                ...
                for(int qx=0; qx<2*qsize; qx++){
                        for(int qy=0; qy<2*qsize; qy++){
                            ...
                            if(((abs(kx*qpi-qx*kpi)!=2*kpi*qpi) && (abs(ky*qpi-qy*kpi)!=2*qpi*kpi) && (abs(kx*qpi-qx*kpi)!=0) && (abs(ky*qpi-qy*kpi)!=0))  && ((qx != qpi) || (qy != qpi)) && ((kx != kpi) || (ky != kpi))){
                                ...
                                nonvertex = (bosefunction(qx*M_PI/qpi,qy*M_PI/qpi,b,1.0/Tbfield)-bosefunction((kx*(M_PI/kpi)-qx*(M_PI/qpi))+M_PI,(ky*(M_PI/kpi)-qy*(M_PI/qpi))+M_PI,b,1.0/Tbfield))/(energy + epsilon);
