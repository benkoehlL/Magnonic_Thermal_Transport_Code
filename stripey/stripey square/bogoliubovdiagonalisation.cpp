...
int main(){
    ...
    for(int k=0;k<n;k++){
        ...
        /*for(int i=0;i<2*N;i++){
            for(int j=0;j<2*N;j++){
                cout << gsl_complex_abs(gsl_matrix_complex_get(helpmatrixH,i,j))*exp(I*gsl_complex_arg(gsl_matrix_complex_get(helpmatrixH,i,j))) << '\t';
            }
            cout << '\n';
        }
        ...
        */
        ...
        /*for(int x=0;x<N;x++){
            for(int y=0;y<N;y++){
                gsl_matrix_complex_set(bogo,x,y+N,gsl_matrix_complex_get(bogo,x+N,y));
                gsl_matrix_complex_set(bogo,x+N,y+N,gsl_matrix_complex_get(bogo,x,y));
            }
        }*/
        for(int x=0;x<N;x++){
            for(int y=0;y<N;y++){
...
void intrastripe(gsl_matrix_complex *edmatrix){
    ...
    for(int i=0;i<N;i++){
        ...
        if(i==0){
            ...
            gsl_matrix_complex_set(edmatrix,i+1,i,
                                   gsl_complex_add(gsl_matrix_complex_get(edmatrix,i+1,i),
                                   gsl_complex_rect(-0.25*t,0.0)));
            gsl_matrix_complex_set(edmatrix,i+1+N,i,
                                   gsl_complex_add(gsl_matrix_complex_get(edmatrix,i+1+N,i),
                                   gsl_complex_rect(0.5*t,0.0)));
    ...
    for(int i=0;i<N;i++){
        for(int j=0;j<N;j++){
            gsl_matrix_complex_set(edmatrix,i+N,j+N, gsl_complex_conjugate(gsl_matrix_complex_get(edmatrix,i,j)));
        }
    }
    for(int i=0;i<N;i++){
        for(int j=0;j<N;j++){
    ...
    gsl_matrix_complex_scale(edmatrix,gsl_complex_rect(-1.0,0.0));
}
...
void interstripe(gsl_matrix_complex *edmatrix, double fourierexponent){
    ...
    for(int i=0;i<N;i++){
        ...
        gsl_matrix_complex_set(edmatrix,i,i,
                               gsl_complex_add(gsl_matrix_complex_get(edmatrix,i,i),
                               gsl_complex_rect(-1.*t+epsilon,0.0)));
        gsl_matrix_complex_set(edmatrix,i,i,
                               gsl_complex_add(gsl_matrix_complex_get(edmatrix,i,i),
                               gsl_complex_polar(0.25*tprime,fourierexponent)));
        gsl_matrix_complex_set(edmatrix,i,i+N,gsl_complex_add(gsl_matrix_complex_get(edmatrix,i,i+N),
                               gsl_complex_polar(0.5*tprime,fourierexponent)));
                gsl_matrix_complex_set(edmatrix,i,i,
                               gsl_complex_add(gsl_matrix_complex_get(edmatrix,i,i),
                               gsl_complex_polar(-0.25*tprime,fourierexponent)));
        gsl_matrix_complex_set(edmatrix,i,i+N,gsl_complex_add(gsl_matrix_complex_get(edmatrix,i,i+N),
